...
| **Standard** | `0.0` (aus) |
| **Bereich** | 0–1 |
| **Wirkung** | Stärke der Mikrorelief-Unterdrückung. Bei > 0 wird das DEM in Makro + Mikro zerlegt: Senken werden aufgefüllt (`depression_fill`), Kammmikrorelief reduziert (`ridge_micro_retention`). 1 = volle Anwendung. |
...
| **Standard** | `0.35` |
| **Bereich** | 0–1 |
| **Wirkung** | Verbleibende Schneedicke auf Hängen oberhalb `max_accumulation_slope_deg`. 0 = kein Schnee auf Steilhang, 1 = volle Basisdicke. |
...
| **Typ** | Zahl (0–1) |
| **Standard** | `0.55` |
| **Wirkung** | In der Signatur vorhanden; aktuell ohne direkte Verwendung im Render-Code (reserviert). |
...
| **Typ** | Zahl (0–1) |
| **Standard** | `0.10` |
| **Wirkung** | In der Signatur vorhanden; aktuell ohne direkte Verwendung im Render-Code. |
...
| **Typ** | Zahl (0–1) |
| **Standard** | `0.34` |
| **Wirkung** | Schneeschicht-Textur in den Snow-Layern (vor Render). |
...
| **Typ** | Zahl (0–1) |
| **Standard** | `0.7` |
| **Wirkung** | In der Config vorhanden; aktuell ohne direkte Verwendung im Render-Code. |
...
| **Typ** | Boolean |
| **Standard** | `true` |
| **Wirkung** | In der Config vorhanden; Wasser wird in `summer_structure` und `map_shading` über `protect_masks` geschützt. |
...
| **Typ** | Zahl (0–1) |
| **Standard** | `0.0` |
| **Wirkung** | Wahrscheinlichkeit für Eis statt offenem Wasser (aktuell nur im Snow-Layer gespeichert). |
...
### `hillshade_strength`, `aspect_strength`

| | |
|---|---|
| **Typ** | Zahl (≥ 0) |
| **Standard** | `0.10` / `0.0` (deaktiviert in neueren Profilen) |
| **Wirkung** | Hillshade- und Expositions-Relief auf Schnee. |
